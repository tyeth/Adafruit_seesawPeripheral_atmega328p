...
void receiveEvent(int howMany) {
  ...
  if (base_cmd == SEESAW_STATUS_BASE) {
    ...
    if (! (VALID_PWM & (1UL << pin))) {
      ...
      // ATmega328P Timer1 16-bit PWM
      // Pin 9 = OC1A, Pin 10 = OC1B
      uint16_t duty_cycle = map(value, 0, 0xFFFF, 0, ICR1);
      if (pin == 9) {
        // Enable OC1A output (COM1A1 = 1, non-inverting mode)
        TCCR1A |= _BV(COM1A1);
        OCR1A = duty_cycle;
      } else if (pin == 10) {
        // Enable OC1B output (COM1B1 = 1, non-inverting mode)
        TCCR1A |= _BV(COM1B1);
        OCR1B = duty_cycle;
      }
#elif defined(MEGATINYCORE)
      // Original ATtiny TCA0 16-bit PWM code
...
#if defined(ATMEGA328_SERIES)
      // ATmega328P Timer1 frequency setting
      // Calculate prescaler and ICR1 value for desired frequency
      // NOTE: changes all PWM outputs on Timer1
      uint8_t prescaler_bits = 0;
      uint16_t prescaler_values[] = {1, 8, 64, 256, 1024};
      unsigned long period;

      for (uint8_t i = 0; i < 5; i++) {
        period = (F_CPU / (unsigned long)prescaler_values[i] / value) - 1;
        if (period <= 65535) {
          prescaler_bits = i + 1;  // CS1x bits: 001=1, 010=8, 011=64, 100=256, 101=1024
          break;
        }
      }
      if (prescaler_bits > 0) {
...
#elif defined(MEGATINYCORE)
      // Original ATtiny TCA0 frequency setting
    ...
    if (module_cmd == SEESAW_KEYPAD_STATUS) {
      // Status register - read only, handled in request
    }
    else if (module_cmd == SEESAW_KEYPAD_EVENT) {
      // Set event configuration for a key
      // Format: [key_number, edge_config]
      // edge_config: (1 << (edge + 1)) | enable
      if (howMany >= 4) {
        uint8_t key = i2c_buffer[2];
        uint8_t edge_cfg = i2c_buffer[3];

        if (key < CONFIG_NUM_KEYPAD_KEYS) {
          // Parse edge config: bit 0 = enable, bits 1-4 = edge enables
          // The CircuitPython driver sends: (1 << (edge + 1)) | enable
          // So edge 0 (HIGH) -> bit 1, edge 1 (LOW) -> bit 2, etc.
          if (edge_cfg & 0x01) {
            // Enable this edge
            g_keypad_edge_config[key] |= (edge_cfg & 0x1E);  // bits 1-4
          } else {
            // Disable this edge
            g_keypad_edge_config[key] &= ~(edge_cfg & 0x1E);
          }
          SEESAW_DEBUG(F("Keypad key "));
          SEESAW_DEBUG(key);
          SEESAW_DEBUG(F(" config: 0x"));
          SEESAW_DEBUGLN(g_keypad_edge_config[key], HEX);
        }
      }
    }
    else if (module_cmd == SEESAW_KEYPAD_INTENSET) {
      // Enable keypad interrupt
      g_keypad_inten = 1;
      SEESAW_DEBUGLN(F("Keypad INT enabled"));
    }
    else if (module_cmd == SEESAW_KEYPAD_INTENCLR) {
      // Disable keypad interrupt
      g_keypad_inten = 0;
      #if CONFIG_INTERRUPT
      Adafruit_seesawPeripheral_clearIRQ();
      #endif
      SEESAW_DEBUGLN(F("Keypad INT disabled"));
    }
    else if (module_cmd == SEESAW_KEYPAD_COUNT) {
      // Count register - read only, handled in request
    }
    else if (module_cmd == SEESAW_KEYPAD_FIFO) {
      // FIFO register - read only, handled in request
    }
  }
#endif
}
